...
class BasicServer(object):
   ...
   def run(self):
      # Use the IPv6 unspecified address for dual-stack support
    addr = ('::', self.port) if socket.has_dualstack_ipv6() else (self.ipaddr, self.port)
    ...
    self._svr.listen(10)
    print(f"server ({addr[0]}) is listening on {self.port}")
   ...
   def run1(self):
        self._svr = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        self._svr.bind((self.ipaddr, self.port))
        print(f"UDP server is listening on {self.port}")

        while self.good:
            try:
                data, addr = self._svr.recvfrom(2048)  # Buffer size of 2048 bytes
                if data:
                    print(f"Received message from {addr}: {data.decode('utf-8')}")
                    csession = SessionHandler(data, addr)
                    csession.start()
                    # Here you would process the data or respond to the client if necessary
                    # For example, to echo back the data you could use:
                    # self._svr.sendto(data, addr)
...
"""
      ...
      if socket.has_dualstack_ipv6():
         ...
         self._svr = socket.create_server(ayddr)
      self._svr.listen(10)
